...
class DataValidation:
    ...
    def is_train_test_file_exists(self)->bool:
        try:
            ...
            if not is_available:
                ...
                testing_file = self.data_ingestion_artifact.test_filepath
    ...
    def validate_dataset_schema(self)->bool:
        try:
            ...
            schema_yaml = read_yaml_file(file_path=schema_file_path)
            columns = "column"
            valid_schema = schema_yaml[columns]

            new_train_schema = {}
            for col in train_df.dtypes:
                new_train_schema[col[0]] = col[1]
            
            list_valid_schema_keys = list(valid_schema.keys())
            list_valid_schema_values = list(valid_schema.values())
            list_new_train_schema_keys = list(new_train_schema.keys())
            list_new_train_schema_values = list(new_train_schema.values())

            if list_valid_schema_keys == list_new_train_schema_keys:
                if list_valid_schema_values == list_new_train_schema_values:
                    x = True
            else:
                x = False
